...
export class UserComponent {
  ...
  public deleteUsuario(id: string): void {
    ...
    of(this.loading.set(true))
      ...
      .subscribe((resp) => {
        ...
        this.MessageService.mensajeSuccess('Usuario eliminado correctamente')
          .then(() => {
            let usuariosFiltros = this.filteredUser.filter((user: any) => user.id !== id);

            console.log("usuario filtradsos", usuariosFiltros);
            this.filteredUser = usuariosFiltros;

            console.log('Usuario nuevos:', this.filteredUser);
            console.log('Llamo al getUsers despues de eliniunar');
  ...
  addUser(user: any): void {
    ...
    var message = this.tipoAction() === 'actualizar' ? 'Usuario actualizado correctamente' : 'Usuario creado correctamente';


    this.MessageService.mensajeSuccess(message)
      .then(() => {
